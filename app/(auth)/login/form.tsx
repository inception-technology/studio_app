...
export default function LoginForm() {
    ...
    useEffect(() => {
        setSelectedLang(normalizeLocaleCode(locale));
    }, [locale]);

    const applyLocale = useCallback(async (nextLocale: string) => {
        const safeLocale = normalizeLocaleCode(nextLocale);
        try {
            window.localStorage.setItem("language_code", safeLocale);
        } catch {
            // no-op
        }
        ...
        await fetch("/api/locale", {
            ...
            body: JSON.stringify({ locale: safeLocale }),
            cache: "no-store",
            credentials: "include",
        }).catch(() => null);

        setSelectedLang(safeLocale);
        setOpenLang(false);
        if (safeLocale !== normalizeLocaleCode(locale)) {
            router.refresh();
        }
    }, [locale, router]);
